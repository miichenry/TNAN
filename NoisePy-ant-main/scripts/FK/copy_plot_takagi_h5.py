...
date_str = "2023-09-30 00:00:00"
date_format = "%Y-%m-%d %H:%M:%S"

stack_method = 'pwd' #'pws', 'linear','robust', 'nroot', 'auto_covariance']
type = 'Allstack_' + stack_method
...
#print(timestamp)
window = f"T{timestamp}" #sys.argv[1]  # "T1662840000"
output_dir = "/srv/beegfs/scratch/shares/cdff/DPM/Postprocessing/FK_takagi"
output_file_time = os.path.join(output_dir, f"ncfs_phase_only_1bit_{stack_method}_ZR-RZtime.jpg")
output_file_fk = os.path.join(output_dir, f"ncfs_phase_only_1bit_{stack_method}_ZR-RZfk.jpg")
...
for ccomp in ['EE', 'EN', 'EZ', 'NE', 'NN', 'NZ', 'ZE', 'ZN', 'ZZ']:
    t1 = time.time()
    data = get_substack_gather_1comp(sfiles, dtype=type, comp=ccomp)
    if not data:
...
for ax in axs.ravel():
    ...
    ax.plot(tsym, tsym * 2000, c="y", lw=2, ls=":")  # Reference line for V=1.5 km/
