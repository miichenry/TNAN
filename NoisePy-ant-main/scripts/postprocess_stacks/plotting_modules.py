...
def plot_substack_cc(sfile,freqmin,freqmax,disp_lag=None,savefig=True,sdir='./'):
    ...
    for spair in spairs:
        ...
        for ipath in path_lists:
            ...
            ax.matshow(data,cmap='gray',extent=[-disp_lag,disp_lag,nwin,0],aspect='auto')
...
def plot_all_moveout(sfiles,dtype,freqmin,freqmax,ccomp,dist_inc,disp_lag=None,savefig=False,sdir=None):
    ...
    lw = 0.6
    # l1, = ax.plot([ndist[-1] / 0.5, 0], [ndist[-1], 0], "k:", lw=lw)
    l2, = ax.plot([ndist[-1] / 2.0, 0], [ndist[-1], 0], c="k", ls="-.", lw=lw)
    l3, = ax.plot([ndist[-1] / 2.5, 0], [ndist[-1], 0], "k--", lw=lw)
    l5, = ax.plot([ndist[-1] / 3.5, 0], [ndist[-1], 0], "k:", lw=lw)
    # ax.plot([-ndist[-1] / 0.5, 0], [ndist[-1], 0], "k:", lw=lw)
    ax.plot([-ndist[-1] / 2.0, 0], [ndist[-1], 0], c="k", ls="-.", lw=lw)
    ax.plot([-ndist[-1] / 2.5, 0], [ndist[-1], 0], "k--", lw=lw)
    ax.plot([-ndist[-1] / 3.5, 0], [ndist[-1], 0], "k:", lw=lw)
    ax.legend([l2, l3, l5], [ "2.0 km/s", "2.5 km/s", "3.5 km/s"], loc='lower right')
