...
def get_stack_gather(sfiles, dtype="Allstack_pws", comp="ZZ", output_fname="dum.npz"):
    ...
    return ncfs #, r0, f, ncts0, t, dt, azimuth, numgood, pairname0
...
if __name__ == "__main__":
    # Parse arguments
    comp = sys.argv[1]  # "RR"
    dtype = "Allstack_robust" # = sys.argv[2]  # "Allstack_pws"
    #datadir = sys.argv[3]  # "/home/users/s/savardg/scratch/riehen/STACK_CHRI_norm/"
    datadir = "/srv/beegfs/scratch/shares/cdff/DPM/Processing/STACK_no_bad_stations_chan_corrected" # = sys.argv[3]
    output_fname = f"/srv/beegfs/scratch/shares/cdff/DPM/Postprocessing/ncfs_{dtype}_{comp}.npz" #sys.argv[4]  # f"riehen_ncfs_wCH_{dtype}_{comp}.npz"
    print(f"comp={comp}, dtype={dtype}, datadir={datadir}, output_fname={output_fname}")
    filelist = glob.glob(os.path.join(datadir, "*", "*.h5"))
    print(f"Number of files: {len(filelist)}")
    _ = get_stack_gather(filelist, dtype=dtype, comp=comp, output_fname=output_fname)
